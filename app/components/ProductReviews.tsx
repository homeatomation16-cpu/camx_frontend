...
function StarDisplay({ rating, size = 14 }: { rating: number; size?: number }) {
  return (
    <span className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => {
        if (rating >= i)
          return <FaStar key={i} size={size} className="text-amber-400" />;
...
function ReviewForm({
  ...
  onSubmit: (review: Omit<Review, "_id" | "helpful">) => void;
}) {
  ...
  if (submitted) {
    return (
      <motion.div
        ...
        className="flex flex-col items-center gap-3 rounded-3xl border bg-green-50 p-10 text-center dark:bg-green-950/20"
      >
    ...
    <div className="rounded-3xl border bg-neutral-50 p-6 dark:bg-card">
          ...
          {[1, 2, 3, 4, 5].map((i) => (
            ...
            >
              {(hovered || rating) >= i ? (
                ...
                <FaRegStar size={28} className="text-neutral-300" />
              )}
            </button>
          ))}
        </div>
      ...
      <div className="mb-4">
        ...
        <input
          ...
          className="w-full rounded-2xl border bg-white px-4 py-3 text-sm outline-none transition focus:border-secondary dark:bg-neutral-900"
        />
      </div>
      ...
      <button
        ...
        disabled={!rating || !title || !body || !author}
        className="flex w-full items-center justify-center gap-2 rounded-2xl bg-secondary py-3 text-sm font-bold text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
      >
        Submit Review
      </button>
    </div>
...
function ReviewCard({ review }: { review: Review }) {
  ...
  return (
    <motion.div
      ...
      className="rounded-3xl border bg-white p-6 dark:bg-card"
    >
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          {review.avatar ? (
            ...
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-secondary/10 text-sm font-black text-secondary">
              {initials}
            </div>
          )}
...
export default function ProductReviews({
  ...
  reviews: Review[];
  avgRating: number;
  onAddReview: (r: Omit<Review, "_id" | "helpful">) => void;
}) {
  ...
  const filtered = reviews.filter((r) =>
    filter ? Math.round(r.rating) === filter : true,
  );

  return (
    <div className="mt-20 border-t pt-14">
      <h2 className="mb-10 text-2xl font-black lg:text-3xl">
        Customer Reviews
      </h2>
      ...
      <div className="grid gap-10 lg:grid-cols-3">
        <div className="lg:col-span-1">
          <div className="sticky top-24 space-y-6">
            <div className="rounded-3xl border bg-neutral-50 p-6 dark:bg-card">
              <div className="mb-1 text-6xl font-black text-secondary">
                {avgRating.toFixed(1)}
              </div>
              ...
              <div className="mt-5 space-y-2">
                {starCounts.map(({ star, count }) => (
                  <button
                    key={star}
                    onClick={() =>
                      setFilter((prev) => (prev === star ? null : star))
                    }
                    className="w-full"
                  >
