...
import { X, User, Mail, Phone, Home, ClipboardList, ShoppingBag, Calendar, FileText } from "lucide-react";
...
export default function ViewOrderInfo({ order }: Props) {
  ...
  return (
    <>
      {/* OPEN BUTTON */}
      <button onClick={() => setIsModalOpen(true)} className="px-4 py-2 rounded-xl bg-secondary text-white text-sm font-bold hover:opacity-90 transition">
        View Info
      </button>
          ...
          <div className="w-full max-w-5xl max-h-[90vh] overflow-hidden bg-white dark:bg-neutral-900 rounded-3xl border border-border flex flex-col">
            ...
            <div className="flex items-start justify-between border-b border-border px-6 py-5">
              <div>
                ...
                <p className="text-sm text-neutral-500 mt-2">{order.orderId}</p>
              </div>

              <button onClick={() => setIsModalOpen(false)} className="w-10 h-10 rounded-xl hover:bg-neutral-100 dark:hover:bg-white/10 flex items-center justify-center">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                ...
                <div className="rounded-3xl border border-border p-6 bg-neutral-50 dark:bg-white/5">
                  <h3 className="font-black text-lg mb-5">Customer Information</h3>
                ...
                {/* ORDER */}
                <div className="rounded-3xl border border-border p-6 bg-neutral-50 dark:bg-white/5">
                  <h3 className="font-black text-lg mb-5">Order Information</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <p className="text-xs text-neutral-500 uppercase mb-2">Status</p>

                      <div className="flex gap-3 items-center flex-wrap">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${getStatusClasses(status)}`}>{status}</span>

                        <select value={status} onChange={(e) => setStatus(e.target.value)} className="h-10 px-3 rounded-xl border border-border bg-transparent outline-none">
                    ...
                    <div>
                      <p className="text-xs text-neutral-500 uppercase">Payment Method</p>

                      <p className="font-semibold">{order.paymentMethod || "COD"}</p>
                    </div>

                    <div>
                      <p className="text-xs text-neutral-500 uppercase">Total Amount</p>
              ...
              <div className="rounded-3xl border border-border p-6 bg-neutral-50 dark:bg-white/5">
                ...
                <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full min-h-32 rounded-2xl border border-border bg-transparent p-4 outline-none resize-none" />
              </div>
              ...
              <div className="rounded-3xl border border-border overflow-hidden">
                ...
                <div className="divide-y divide-border">
                  {order.items?.map((item: OrderItem, index: number) => (
                    <div key={index} className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 p-5">
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-neutral-500 mt-1">Product ID: {item.productId}</p>
                      ...
                      <div className="text-left lg:text-right">
                        <p className="font-black text-xl text-secondary">{formatCurrency(item.unitPrice * item.quantity)}</p>

                        <p className="text-sm text-neutral-500 mt-1">{formatCurrency(item.unitPrice)} each</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="px-6 py-5 border-t border-border flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 bg-neutral-50 dark:bg-white/5">
              ...
              <div className="flex gap-3">
                <button onClick={() => setIsModalOpen(false)} className="h-12 px-6 rounded-2xl border border-border font-bold">
                  Close
                </button>

                {(order.notes !== notes || order.status !== status) && (
                  <button onClick={updateOrder} className="h-12 px-6 rounded-2xl bg-secondary text-white font-black">
                    Save Changes
                  </button>
                )}
              </div>
