...
export default function ProductCard({ product, onAddToCart }: Props) {
  ...
  const safeImg = (src?: string) =>
    src && !src.includes("example.com") ? src : "/placeholder.jpg";
  ...
  return (
    ...
    >
      ...
      <Link
        href={href}
        className="relative block h-52 overflow-hidden bg-neutral-50 dark:bg-neutral-900 sm:h-60"
      >
  ...
  "
        ...
        <div className="absolute inset-0 flex items-center justify-center gap-3 bg-black/0 opacity-0 transition-all duration-300 group-hover:bg-black/10 group-hover:opacity-100">
          ...
          <button
            ...
            disabled={!inStock}
            className="flex h-9 w-9 items-center justify-center rounded-full bg-white text-neutral-700 shadow-lg transition hover:scale-110 hover:bg-secondary hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
          >
        ...
        <button
          ...
          className="absolute right-3 top-3 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-white/90 shadow-md backdrop-blur-sm transition-all hover:scale-110 dark:bg-black/60"
        >
          <motion.svg
            viewBox="0 0 24 24"
            width={15}
        ...
        {/* DISCOUNT */}
        {discount && (
          <div className="absolute left-3 top-3 rounded-lg bg-secondary px-2 py-1 text-[10px] font-black uppercase tracking-wider text-white shadow-md">
            -{discount}%
          </div>
        )}
            ...
            {product.images.slice(0, 4).map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all ${
                  imgIndex === i ? "w-4 bg-white" : "w-1.5 bg-white/60"
                }`}
              />
            ))}
          </div>
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        {/* NAME */}
        <Link href={href}>
          <h3 className="line-clamp-2 min-h-10 text-[13px] font-semibold leading-5 text-neutral-800 transition hover:text-secondary dark:text-white">
            {product.name}
          </h3>
        </Link>
            ...
            <>
              <div
                className={`h-1.5 w-1.5 rounded-full ${
                  lowStock ? "bg-amber-400" : "bg-green-400"
                }`}
              />

              <span
                className={`text-[11px] font-medium ${
                  lowStock ? "text-amber-500" : "text-green-500"
                }`}
              >
        ...
        <motion.button
          ...
          disabled={!inStock}
          whileTap={{ scale: 0.97 }}
          className={`relative mt-4 flex h-10 w-full items-center justify-center gap-2 overflow-hidden rounded-xl text-[13px] font-bold text-white transition-all duration-300 ${
            inStock
