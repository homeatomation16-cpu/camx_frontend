...
function FilterChip({
  ...
  return (
    <motion.span
      ...
      className="flex items-center gap-1.5 rounded-full border border-secondary/30 bg-secondary/8 px-3 py-1.5 text-[11px] font-semibold text-secondary"
    >
      {label}
...
export default function ProductsPage() {
  ...
  const brands = useMemo(
    () => [...new Set(products.map((p) => p.brand || "Other"))],
    [products],
  );
  ...
  const activeFilters = useMemo(() => {
    ...
    selectedBrands.forEach((b) =>
      chips.push({ label: b, onRemove: () => toggleBrand(b) }),
    );
  ...
  const filteredProducts = useMemo(() => {
    let result = products.filter((p) => {
      ...
      const matchBrand =
        selectedBrands.size === 0 || selectedBrands.has(p.brand || "Other");
  ...
  const sidebarContent = (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        {activeFilters.length > 0 && (
          <button
            ...
            className="text-[11px] font-semibold text-neutral-400 hover:text-secondary transition"
          >
            Clear all
          </button>
        )}
      ...
      <div>
        <h3 className="mb-4 text-[11px] font-black uppercase tracking-widest text-neutral-400">
          Price Range
        </h3>
        <PriceRangeSlider
          min={0}
    ...
    <main className="min-h-screen bg-background px-4 pb-24 pt-28 sm:px-6">
      <div className="mx-auto max-w-425">
        ...
        <div className="mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="mb-1 text-xs font-black uppercase tracking-widest text-secondary">
              CAMX Store
            </p>
          ...
          <div className="relative w-full lg:w-96">
            ...
            <input
              ...
              className="h-12 w-full rounded-2xl border border-neutral-200 bg-white pl-11 pr-10 text-sm outline-none transition focus:border-secondary dark:border-border dark:bg-card"
            />
            <AnimatePresence>
              {search && (
                <motion.button
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl border border-neutral-200 bg-white px-4 py-2.5 text-xs font-bold dark:border-border dark:bg-card lg:hidden"
                >
                  <SlidersHorizontal size={13} />
                  Filters
                  {activeFilters.length > 0 && (
                    <span className="flex h-4 w-4 items-center justify-center rounded-full bg-secondary text-[9px] font-black text-white">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  <select
                    ...
                    className="h-10 appearance-none rounded-xl border border-neutral-200 bg-white pl-4 pr-9 text-xs font-semibold outline-none transition focus:border-secondary dark:border-border dark:bg-card"
                  >
            ...
            {loading ? (
              ...
              <motion.div
                ...
                className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-neutral-200 py-28 text-center dark:border-border"
              >
                ...
                <button
                  onClick={clearFilters}
                  className="mt-5 rounded-xl bg-secondary px-5 py-2.5 text-sm font-bold text-white transition hover:opacity-90"
                >
                  Clear Filters
                </button>
              </motion.div>
        ...
        {mobileSidebarOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-y-0 left-0 z-50 w-80 overflow-y-auto bg-white p-6 dark:bg-card lg:hidden"
            >
              ...
              <button
                onClick={() => setMobileSidebarOpen(false)}
                className="mt-8 w-full rounded-2xl bg-secondary py-3 text-sm font-bold text-white"
              >
